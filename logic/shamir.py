...
def random_poly(secret, t, GF):
    ...
    coeffs[0] = GF(secret)
    return galois.Poly(coeffs, field=GF,order="asc")  # a0 + a1 x + ... + a_{t-1} x^{t-1}
...
def make_shares(secret: int, n: int, t: int, p: int):
    ...
    """
    if not (2 <= t <= n):
        raise ValueError("Need 2 <= threshold <= n")
    GF = gf(p)
    f = random_poly(secret, t, GF)
    xs = GF.Range(1, n + 1)  # 1..n
    ys = f(xs)
    return GF, f, xs, ys
...
def lagrange_interpolate_at_zero(xs, ys, GF):
    ...
    Works for any number of points; correct only if >= threshold points from same polynomial.
    """
    x0 = GF(0)
    s = GF(0)
    for i in range(len(xs)):
        # basis L_i(0) = Π_{j≠i} (0 - x_j)/(x_i - x_j)
        num = GF(1)
        den = GF(1)
